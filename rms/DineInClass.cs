...
namespace rms
{
    class DineInClass : Connection
    {
        public DataTable getDineInOrdersList()
        {
            openConnection();
            string mysql = "SELECT table_no, orders.id AS order_id FROM customer, orders WHERE customer.id = orders.cust_id AND orders.order_type = 'Dine-in' AND is_completed = 0 ORDER BY customer.id ASC";
